...
export default function NameEditorPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const data = rawData as ProfileDisplayNameRow | null;
  ...
  async function handleSave() {
    ...
    const { error: updateError } = await updateOwnProfile({ display_name: displayName.trim() || null });

    setSaving(false);

    if (updateError) {
      setError(updateError || "Failed to save name");
      return;
    }
