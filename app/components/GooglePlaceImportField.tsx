...
type GooglePlaceImportFieldProps = {
  userId: string;
  placeId: string;
...
export default function GooglePlaceImportField({
  userId,
  placeId,
  onImportSuccess,
  compact = false,
}: GooglePlaceImportFieldProps) {
  ...
  async function handleImport() {
    if (!googleUrl.trim() || !userId || !placeId) return;
        ...
        {showForm && (
          <div className="space-y-2">
            <input
              ...
              placeholder="Вставьте ссылку Google Maps"
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#6F7A5A]">
            Вставьте ссылку Google Maps для автоматического заполнения
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="space-y-2">
          <div>
            <label className="block text-xs font-medium text-[#1F2A1F] mb-1.5">
              Ссылка Google Maps (необязательно)
            </label>
            <input
              type="url"
              value={googleUrl}
          ...
          {importSuccess && (
            <div className="rounded-xl border border-green-200 bg-green-50/50 p-3 text-sm text-green-700 flex items-center gap-2">
              ...
              <span>Импортировано из Google</span>
            </div>
          )}
