/**
 * PATCH /api/admin/users/[id]/role-v3
 *
 * Standalone admin role/plan updater. This intentionally does not import the
 * older role route so deployed serverless bundles cannot reuse stale payload
 * code. The update body is allowlisted and never includes profiles.updated_at.
 */
...
const jsonError = (message: string, status: number, code?: string) =>
  NextResponse.json(
    { error: `[${ROUTE_VERSION}] ${message}`, code, routeVersion: ROUTE_VERSION },
    {
      status,
      headers: { "x-maporia-admin-role-route": ROUTE_VERSION },
    }
  );
