...
Linked places writes go through `/api/place-links`. `POST /api/place-links` verifies the Bearer session, validates that the parent is a `location` and the child is a `service` or `experience`, and lets the child owner create either an `active` same-owner link or a `pending` cross-owner request; admins can create active links. `PATCH /api/place-links/[id]` lets the location owner/admin approve or reject pending requests, and `DELETE /api/place-links/[id]` lets either listing owner/admin remove the link. This keeps the browser UI out of direct `place_links` writes while preserving the documented approval model.

Map discovery uses those active `place_links` as a coordinate fallback: if a service/experience has no usable `lat/lng`, `/map` keeps the card as its primary `kind` but places its marker at the linked parent location.
